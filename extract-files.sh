...
extract "$MY_DIR/aoscp-proprietary-blobs.txt" "$SRC"
...
if [ "$SRC" != "adb" ]; then
  for file in $(egrep -v '(^#|^$)' "$MY_DIR"/aoscp-proprietary-blobs-vendorimg.txt); do
...
extract "$MY_DIR/aoscp-proprietary-blobs-vendorimg.txt" "$SRC"
